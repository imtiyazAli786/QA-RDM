...
mkdir -p build_cache
swiftc -O -module-cache-path build_cache -framework Cocoa -framework WebKit main.swift -o "$MACOS_DIR/$APP_NAME"

# 7. Ad-hoc codesign to satisfy macOS gatekeeper locally
codesign -s - --force "$BUNDLE_DIR" 2>/dev/null || true

# 8. Install into /Applications/ so it shows in Launchpad & Spotlight
echo "📦 Installing into /Applications/..."
pkill -f "CalendarWidget" 2>/dev/null || true
rm -rf "/Applications/$BUNDLE_DIR"
cp -R "$BUNDLE_DIR" "/Applications/"
rm -rf "$BUNDLE_DIR" # Remove local copy so Spotlight only indexes /Applications

echo "✅ Build complete and installed to /Applications/$BUNDLE_DIR"
